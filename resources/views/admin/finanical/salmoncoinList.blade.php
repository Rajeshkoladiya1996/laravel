<table id="salmonCoinList" class="table table-striped table-bordered dt-responsive nowrap mytabel" style="width:100%">
    <thead>
        <tr>
            <th>Min THB</th>
            <th>Max THB</th>
    ...
    <tbody>
        @foreach($topuprates as $data)
            <tr>
                <td>{{$data->from_price}}</td>
                <td>
                    @if($loop->last)
                        More Than {{$data->from_price}}
                    @else
                        {{$data->to_price}}
                    @endif
                </td>
                <td>{{$data->rate}}</td>
                <!-- <th><p>{{$data->is_active}}</p></th> -->
                <td>
                    <ul class="action-wrap">
                        <li>
                            <a href="javascript:void(0)" class="editSalmonCoin" id="editSalmonCoin" data-id="{{$data->id}}">
                        ...
                        <li>
                            <a href="javascript:void(0)" id="deleteSalmonCoin" data-id="{{$data->id}}" data-type="salmonCoinlist">

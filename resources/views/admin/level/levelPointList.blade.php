<table id="example2" class="table td-wrap-table table-striped table-bordered dt-responsive nowrap mytabel" style="width:100%">
    <thead>
      <tr>
        <th>No</th>
        <th>Category Name</th>
    ...
    <tbody>
      @php
          $i=0;
      @endphp
      @foreach ($levelPointsList as $data)
        <tr id="del_{{$data->id}}">
          <td>{{ ++$i}}</td>
          <td>{{ Str::ucfirst($data->category)}}</td>
          <td>{{$data->name}}</td>
          <td>{{$data->points}}</td>
          <td>{{$data->per_day}}</td>
          <td>{{$data->description}}</td>
          <td>{!! ($data->status == '1')? '<span class="green level-point-status" data-status='.$data->status.' data-id='.$data->id.'>Active</span>': '<span class="red level-point-status" data-status='.$data->status.' data-id='.$data->id.'>De-active</span>' !!}</td>
          <td>
            <ul class="action-wrap">
                <li>
                    <a href="{{route('admin.level.point.edit',$data->id)}}" >
                ...
                <li>
                    <a href="javascript:void(0)" id="deleteLevel" data-id="{{$data->id}}" data-type="levelpoint">

...
<table id="avtarcategoryTable" class="table table-striped table-bordered dt-responsive mytabel" style="width:100%">
    ...
    <tbody> 
        @foreach ($avtarAttr as $i => $data)
      
                <td>{{ ++$i }}</td>
                <td>{{ $data->avtartype->name }}</td>
                <td>{{ $data->component_id }}</td>
                <td>{{ $data->avtarcategory->name }}</td>
                <td>
                    @if(isset($data->image))
                    <img src="{{URL::to('storage/app/public/uploads/avtar/component/'.$data->image)}}" alt="" width="50px" height="50px">
                    @else
                     -
                    @endif
                </td>
                <td>{{ $data->iscolor }}</td>
                <td> <span class="{{($data->status == '1')?'green' : 'red' }} avtar-component-status" data-status="{{$data->status}}" data-id="{{$data->id}}">{!! ($data->status == '1')? 'Active' : 'De-active' !!}</span> </td>
                <td>
                    <ul class="action-wrap">
                        <li>
                            <a href="javascript:void(0)" id="editcomponent" data-id="{{$data->id}}">
                        ...
                        <li>
                            <a href="javascript:void(0)" id="deletecomponent" data-id="{{$data->id}}" data-type="avtarlist">
